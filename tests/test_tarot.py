...
import json
from plugins.data import load

class TestTarot(unittest.TestCase):
    imageset = Image.open('../materials/tarot-waite.png')
    keywords = load('../materials/keywords.json')

    def test_tarot_card_numbers(self):
        deck = Deck(shuffled=False, imageset=self.imageset, keywords=self.keywords)
    ...
    def test_card_image(self):
        deck = Deck(shuffled=True, imageset=self.imageset, keywords=self.keywords)
